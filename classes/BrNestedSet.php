...
class BrNestedSet extends BrObject
{
  ...
  public function verify()
  {
    ...
    ')) {
      throw new BrAppException(self::ERROR_NESTED_SET_IS_BROKEN);
    }
    if ($row = br()->db()->getRow('
      SELECT COUNT(1) amount, MIN(left_key) min_left, MAX(right_key) max_right
        FROM ' . $this->tableName
    )) {
      if ($row['amount']) {
        if ($row['min_left'] != 1) {
  ...
  public function internalSetup($key = null, $left = 0, $level = 0, $check_only = false)
  {
    ...
    if (strlen($key)) {
      ...
      $sql = '
        SELECT ' . $this->keyField . '
          FROM ' . $this->tableName . '
         WHERE ' . $this->parentField . ' IS NULL';
    }
    if ($this->orderField) {
  ...
  public function getNodePosition($values)
  {
    ...
    if ($this->orderField) {
      $parentFilter =
        br($values, $this->parentField) ?
          br()->placeholder($this->parentField . ' = ?', $values[$this->parentField]) :
          $this->parentField . ' IS NULL';

      $sql = br()->placeholder('
        SELECT right_key + 1 left_key
             , level - 1 level
          FROM ' . $this->tableName . '
         WHERE ' . $parentFilter . '
